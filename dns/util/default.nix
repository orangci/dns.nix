...
let
  ...
  writeCharacterString =
    s:
    if stringLength s <= 255 then
      ''"${s}"''
    else
      concatMapStringsSep " " (x: ''"${x}"'') (splitInGroupsOf 255 s);
  ...
  align4BytesOrExpand =
    n: v: if v == "" then (lib.fixedWidthString (4 * n) "0" "") else align4Bytes v;
  ...
  mkIPv6ReverseRecord =
    v6: lib.concatStringsSep "." (lib.reverseList (mkIPv6RecordAux v6)) + ".ip6.arpa";

  # : str -> str
  # Returns the reversed record of the ipv4
  mkIPv4ReverseRecord =
    v4: lib.concatStringsSep "." (lib.reverseList (lib.splitString "." v4)) + ".in-addr.arpa";
in
{
